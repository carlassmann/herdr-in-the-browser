...
interface TerminalViewOptions {
  ...
  onReady(view: TerminalView): void;
  onError(): void;
}
...
export function attachTerminalView(
  shell: HTMLElement,
  {
    label,
    controls,
  ...
  void (async () => {
    ...
    disposeWithEffect(
      attachMouseReporting(container, openedTerminal, sendInput),
    );
    fitAddon.fit();
    ...
    onReady({
      ...
      size() {
        return { cols: openedTerminal.cols, rows: openedTerminal.rows };
      },
      focus() {
        openedTerminal.focus();
      },
